...
# Ventana estilo telefono SOLO en escritorio: en Android/iOS se respeta la
# pantalla real del dispositivo (forzar el tamano rompe el layout del APK).
try:
    from kivy.utils import platform as _platform
except Exception:
    _platform = "unknown"

IS_MOBILE = _platform in ("android", "ios")
...
if not IS_MOBILE:
    ...
    try:
        # El teclado no debe tapar los campos de texto en el telefono.
        Window.softinput_mode = "below_target"
    except Exception as _e:
        print("[UI] softinput_mode no disponible:", _e)
...
# En Android el APK es de solo lectura: crear la carpeta puede fallar y no debe
# tumbar la aplicacion.
try:
    os.makedirs(IMG_DIR, exist_ok=True)
except Exception as _e:
    print("[UI] Carpeta de imagenes de solo lectura:", _e)
...
class OcupamorApp(App):
    ...
    def build(self):
        ...
        # Boton "atras" de Android: vuelve al menu en vez de cerrar la app.
        Window.bind(on_keyboard=self._on_key)
        return self.sm

    def _on_key(self, _window, key, *_args):
        if key in (27, 1001):  # ESC / boton atras
            actual = self.sm.current
            if actual in ("login", "menu"):
                return False  # deja salir de la app
            if actual in ("abecedario", "monosilabas", "silabas",
                          "vocabulario"):
                self.sm.current = "inicial_hub"
            elif actual in ("num_numeros", "num_sumas", "num_restas",
                            "num_figuras", "num_series"):
                self.sm.current = "matematicas"
            elif actual == "mission_play":
                self.sm.current = "misiones"
            elif actual == "inicial_hub":
    ...
    def on_pause(self):
        """Android: al minimizar, la app se conserva en memoria."""
        try:
            speech.stop()
        except Exception:
            pass
